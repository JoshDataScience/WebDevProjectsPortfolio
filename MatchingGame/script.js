...
function appendNewCard(parentElement) {
  ...
  //everytime we use card element, we are making a div
  // Step 2: Add the "card" class to the card element.
  //at the same time, add the card class to that div
...
function shuffleCardImageClasses() {
  ...
  shuffledImages=_.shuffle(cardImages)
...
function createCards(parentElement, shuffledImageClasses) {
  ...
  // we chose 12 because it is 6*2, the number of cards doubled. If we had 10 cards, we'd pick 20 here
  for (let i = 0; i < 12; i++) {
  ...
  //     index: i,
  //     element: 
  //     imageClass: shuffleCardImageClasses[i]
  //     }
  ...
  // }      
    ...
    // shuffleCardImageClasses[i]
    //add image to classlist
...
function onCardFlipped(newlyFlippedCard) {
  ...
  // Step 3: If the cards don't match, then remove the "flipped" class from each, reset 'lastCardFlipped', and return.
    ...
    // console.log(doCardsMatch(lastCardFlipped, newlyFlippedCard));
    //this line should always return false because the  if statement is always false for doCardsMatch -ie only print false on non-matches
  ...
  lastCardFlipped = null; //only want 1 card flipped at a time
...
runAllTests();
//do not run the tests individually and then run all at the same time because it will reset stuff - only keep runalltests() at the end!!
