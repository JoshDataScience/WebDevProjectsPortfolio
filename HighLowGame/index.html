...
function setupNewGame() {
  ...
  console.log("random # is :" + magicNumber);
  remainingGuesses = 5;
...
function handleGuess() {
  ...
  if (remainingGuesses == -1){ //out of guesses, start a new game
    setupNewGame();
  }
  if(remainingGuesses <= 0) { //if you still have 0 guesses, you lose and start over
    return;
  }
  ...
  if (guess === magicNumber){
    ...
    remainingGuesses = 0;
    showRemainingGuesses(remainingGuesses); //tell the HTML to update it on the page
    return;
  }
